/* ----------------------------------------------
 ...
 The program checks the user's inputs in the following ways:
    1) Their ID must be four digits.
...
int main(void)
{
    ...
    while (flag == 0)  // use while loop to keep asking for user to input correctly, while input is not in proper bounds
    {
        ...
        if (id_num > 9999 || id_num < 1000)
        {
            printf("ID number must be 4 digits, retry\n");
        }
        else
        {
    ...
    {
        printf("Enter credit limit (no commas): $");
        scanf("%lf", &credit_lim);
    ...
    // Now ask for current balance from user: this has no restrictions
    printf("Enter current balance (no commas): $");
